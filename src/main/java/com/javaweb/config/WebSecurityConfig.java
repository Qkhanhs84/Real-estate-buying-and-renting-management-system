...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
                http.csrf().disable()
                .authorizeRequests()
                        .antMatchers(HttpMethod.POST,"/api/buildings/staffs").hasRole("MANAGER")
                        .antMatchers("/api/user").hasRole("MANAGER")
                        .antMatchers(HttpMethod.DELETE, "/api/buildings/{ids}","/api/transactions/{id}").hasRole("MANAGER")
                        .antMatchers(HttpMethod.POST, "/api/transactions").hasAnyRole("MANAGER","STAFF")
                        .antMatchers(HttpMethod.GET, "/admin/user-list","/admin/user-edit","/admin/profile-{username}",
                                "/admin/user-edit-{id}","/admin/profile-password").hasRole("MANAGER")
                        .antMatchers("/admin/customers/{id}/staffs", "/admin/customers/staffs","/admin/customers/{ids}").hasRole("MANAGER")
